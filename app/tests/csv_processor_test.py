...
from fastapi_celery.processors.file_processors.csv_processor import CSVProcessor
...
class TestCSVProcessor(unittest.TestCase):
    ...
    @patch(
        "fastapi_celery.processors.file_processors.csv_processor"
        ".PODataParsed"
    )
    @patch(
        "fastapi_celery.processors.file_processors.csv_processor"
    ...
    def test_parse_file_to_json(
        ...
        mock_tracking.file_path = self.dummy_path
        mock_tracking.project = "test_project"

        with patch("builtins.open", new=unittest.mock.mock_open(read_data=self.sample_csv)):
            processor = CSVProcessor(tracking_model=mock_tracking, source=SourceType.LOCAL)
